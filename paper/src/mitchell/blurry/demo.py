...
def demo_mitchell():
    ...
    return MitchellSimulation(Nx, Ny, deltaT, deltaX, D, D, t_in=0.3, t_out=6.0, t_close=150, t_open=50, v_gate=0.13,)
...
def update_new(data):
    ...
    if (mode):
        ...
        newData = gaussian_filter(input=sim._v, sigma=5, truncate=8)
    mat.set_data(newData)#sim._v if mode else sim._h)
...
class StorageCallback(object):
    ...
    def switch_mode(self, event):
        ...
        bmode.label.set_text("v" if mode else "v blurred")
