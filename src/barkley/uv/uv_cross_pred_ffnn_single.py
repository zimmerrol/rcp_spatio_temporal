...
input_l = Input(shape=(len(input_x),))
first_layer = Dense(len(input_x), activation='relu')(input_l)
output_l = Dense(1, activation='relu')(first_layer)

model = Model(input_l, first_layer, output_l)
...
for y in range(150):
    ...
    for x in range(150):
        ...
        model.fit(flat_v_data_train, flat_u_data_train, epochs=50, batch_size=100, shuffle=True, validation_data=(flat_v_data_test, flat_u_data_test))
        score = model.evaluate(flat_v_data_test, flat_u_data_test, batch_size=100)
        print(score)
