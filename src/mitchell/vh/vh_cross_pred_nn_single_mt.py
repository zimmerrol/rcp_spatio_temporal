...
def predict_inner_pixel(data, def_param=(shared_v_data, shared_u_data)):
    ...
    shared_delayed_v_data = create_2d_delay_coordinates(shared_v_data[:, y-patch_radius:y+patch_radius+1, x-patch_radius:x+patch_radius+1], ddim, tau=119)
...
def mainFunction():
    ...
    u_data = None
    print("generating data...")
    u_data_t, v_data_t = generate_data(ndata, 20000, 50, Ngrid=N)#20000 was 50000 #, delay_dimension=ddim, patch_size=sigma)
    ...
    pool = Pool(processes=8, initializer=get_prediction_init, initargs=[queue,])
