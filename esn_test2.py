...
def sineshit():
    ...
    esn = ESN(n_input=1, n_output=1, n_reservoir=200, random_seed=42, noise_level=0.001, leak_rate=0.7, spectral_radius=1.35, sparity=0.1)
    esn.fit(inputData=y[:5000, :], outputData=y[:5000,:], transient_quota=0.4)

    Y = esn.generate(n=15000, continuation=True, initial_input=y[5000,:])

    print(x[5000:].shape)
    print(Y[0,:].shape)
    #plt.plot(x,y[:,0], linestyle="--")
    #plt.plot(x[5000:],Y[0,:], linestyle=":")
...
esn = STESN(n_input=12, n_output=9, n_reservoir=1000, leak_rate=0.8, spectral_radius=0.9, random_seed=42, noise_level=0.01, sparity=0.30)
